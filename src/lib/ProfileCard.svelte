<script lang="ts">
	...
	export let miniAvatarUrl: string | undefined = undefined;
	export let name: string = ''; // 'Javi A. Torres';
	...
	const handlePointerMove = (event: PointerEvent) => {
		if (!cardRef || !wrapRef || !enableTilt) return;

		const rect = cardRef.getBoundingClientRect();
		updateCardTransform(event.clientX - rect.left, event.clientY - rect.top, cardRef, wrapRef);
	};
	...
	const handleMiniImageError = (event: Event) => {
		const target = event.target as HTMLImageElement;
		target.style.opacity = '0.5';
		target.src = avatarUrl;
	};

	// Reactive statement for card styles
	$: cardStyle = {
	...
	onMount(() => {
		if (!enableTilt || !cardRef || !wrapRef) return;
		...
		updateCardTransform(initialX, initialY, cardRef, wrapRef);
		createSmoothAnimation(ANIMATION_CONFIG.INITIAL_DURATION, initialX, initialY, cardRef, wrapRef);

		return () => {
			if (cardRef) {
				cardRef.removeEventListener('pointerenter', handlePointerEnter);
				cardRef.removeEventListener('pointermove', handlePointerMove);
				cardRef.removeEventListener('pointerleave', handlePointerLeave);
			}
			cancelAnimation();
		};
	});
...
<div
	...
	style={Object.entries(cardStyle)
		.map(([key, value]) => `${key}: ${value}`)
		.join('; ')}
>
	<section bind:this={cardRef} class="pc-card">
		<div class="pc-inside">
...
<style>
	...
	:global(.pc-card-wrapper.active)::before {
